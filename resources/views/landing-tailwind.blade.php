...
@section('main')
    ...
    <nav class="bg-gray-800 bg-white/5 backdrop-blur-md shadow-lg">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4 flex justify-between items-center">
            ...
            <div>
                ...
                <div class="relative inline-block text-left">
                    ...
                    >
                        ...
                        <form method="POST" action="{{ route('auth.logout') }}">
                            @csrf
                            <button type="submit" class="w-full text-left px-4 py-2 hover:bg-red-500/20 transition flex text-red-500">
                ...
                @else
                <a href="{{route('auth.login.form')}}" class="flex items-center gap-2 px-5 py-2 rounded-lg bg-white/10 text-white border border-white/20 hover:bg-white/20 transition">
    ...
    <section class="flex flex-col justify-center items-center text-center h-[calc(100vh-80px)] px-4">
        ...
        <div class="flex gap-4">
            <a href="#pros" class="flex items-center gap-2 px-5 py-2 rounded-lg bg-white/10 text-white border border-white/20 hover:bg-white/20 transition">
                <i data-lucide="handshake" class="w-5 h-5"></i>
                <span class="leading-none">Why Choose Us?</span>
            </a>
            @if (Auth::check())
            <a href="{{route('dashboard')}}" class="flex items-center gap-2 px-5 py-3 rounded-lg bg-white/10 text-white border border-white/20 hover:bg-white/20 transition">
                <i data-lucide="cog" class="w-5 h-5"></i>
                <span class="leading-none">Visit Dashboard</span>
            </a>
            
            @else
            <a href="{{route('auth.login.form')}}" class="flex items-center gap-2 px-5 py-3 rounded-lg bg-white/10 text-white border border-white/20 hover:bg-white/20 transition">
                <i data-lucide="log-in" class="w-5 h-5"></i>
                <span class="leading-none">Login</span>
            </a>
            @endif
    ...
    <section id="pros" class="relative z-20 py-16 mb-20">
        <div class="container mx-auto px-4 text-center">
            ...
            <div class="grid md:grid-cols-3 gap-8">
                <div class="bg-gray-800 bg-white/8 backdrop-blur-md shadow-lg rounded-xl p-6 hover:bg-white/10 transition duration-400 cursor-pointer">
